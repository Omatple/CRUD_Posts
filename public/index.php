...
use App\Utils\AlertNotification;
...
if (($user = $_SESSION["user"] ?? false) && isset($_POST["id"])) {
    ...
    if ($postId && ($post = Post::fetchPostById($postId))) {
        $newStatus = ($post["status"] === PostStatus::Published->getStatusLabel())
            ? PostStatus::Draft->getStatusLabel()
            : PostStatus::Published->getStatusLabel();
        Post::updatePostStatus($postId, $newStatus);
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <main class="px-8 mx-auto">
        <div class="mt-4 w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            <?php
            foreach (Post::fetchAllPosts() as $post):
                if ($user || PostValidator::isPostPublished($post["status"])):
                    $categoryColor = BookCategories::getCategoryColor($post["name"]);
                    $bgPostColor = ($post["status"] === PostStatus::Published->getStatusLabel()) ? "green" : "red";
            ?>
                    <article class="w-full h-80 p-2 border-2 rounded-xl shadow-xl border-blue-600 bg-<?= $bgPostColor ?>-100 relative">
                        <div class="flex flex-col justify-center w-full h-full">
                            ...
                            opacity-50"><?= htmlspecialchars($post["title"]) ?></h1>
                            </div>
                            <div class="mt-4">
                                <p class="italic"><?= htmlspecialchars($post["content"]) ?></p>
                            </div>
                            <div class="mt-4 w-1/4 mx-auto">
                                <p class="font-bold text-white text-center px-2 py-1 rounded-xl bg-<?= $categoryColor ?>-500">
                                    <?= htmlspecialchars($post["name"]) ?></p>
                            </div>
                            <?php if ($user): ?>
                                <form class="mt-4" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="POST">
...
<?= AlertNotification::displayAlert() ?>
